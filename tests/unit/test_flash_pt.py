...
class TestSinglePhaseDetection:
    ...
    def test_single_phase_pure_component(self, flash, pure_methane):
        ...
        result = flash.calculate(
            z, temperature=300, pressure=5e6, critical_temperatures=tc, critical_pressures=pc
        )
...
class TestKValueInitialization:
    ...
    def test_k_values_positive(self, flash, binary_ethane_propane):
        ...
        z, tc, pc = binary_ethane_propane
        result = flash.calculate(
            z, temperature=300, pressure=2e6, critical_temperatures=tc, critical_pressures=pc
        )
...
class TestFlashInputValidation:
    ...
    def test_invalid_temperature(self, flash, binary_ethane_propane):
        ...
        with pytest.raises(ValueError, match="Temperature must be positive"):
            flash.calculate(
                z, temperature=0, pressure=2e6, critical_temperatures=tc, critical_pressures=pc
            )

        with pytest.raises(ValueError, match="Temperature must be positive"):
            flash.calculate(
                z, temperature=-100, pressure=2e6, critical_temperatures=tc, critical_pressures=pc
            )
    ...
    def test_invalid_pressure(self, flash, binary_ethane_propane):
        ...
        with pytest.raises(ValueError, match="Pressure must be positive"):
            flash.calculate(
                z, temperature=300, pressure=0, critical_temperatures=tc, critical_pressures=pc
            )

        with pytest.raises(ValueError, match="Pressure must be positive"):
            flash.calculate(
                z, temperature=300, pressure=-1e6, critical_temperatures=tc, critical_pressures=pc
            )
    ...
    def test_list_composition_converted_to_array(self, flash, binary_ethane_propane):
        ...
        result = flash.calculate(
            z_list, temperature=300, pressure=2e6, critical_temperatures=tc, critical_pressures=pc
        )
