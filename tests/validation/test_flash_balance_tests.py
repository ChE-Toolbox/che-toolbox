...
class TestBinaryEthanePropaneLiterature:
    ...
    def test_ethane_propane_300k_2mpa_standard_case(
        self, flash, compound_critical_properties
    ):
        ...
        if result.convergence == FlashConvergence.SUCCESS:
            ...
            assert abs(result.L - L_ref) / L_ref < 0.10, \
                f"L = {result.L} vs ref {L_ref}"
            assert abs(result.V - V_ref) / V_ref < 0.10, \
                f"V = {result.V} vs ref {V_ref}"
    ...
    def test_ethane_propane_310k_2p5mpa(
        self, flash, compound_critical_properties
    ):
        ...
        result = flash.calculate(z=z, temperature=T, pressure=P,
                                critical_temperatures=tc, critical_pressures=pc)
...
class TestBinaryMethanePropaneLiterature:
    ...
    def test_methane_propane_280k_3mpa(
        self, flash, compound_critical_properties
    ):
        ...
        if result.convergence == FlashConvergence.SUCCESS:
            ...
            assert result.y[0] > result.x[0], \
                "Methane should be more concentrated in vapor"
    ...
    def test_methane_propane_300k_4mpa(
        self, flash, compound_critical_properties
    ):
...
class TestFlashMultipleConditionsNIST:
    ...
    @pytest.mark.parametrize("test_case", [
        pytest.param(
            {
                'comp1': 'ethane', 'comp2': 'propane',
                'z': np.array([0.60, 0.40]),
                'T': 300.0, 'P': 2e6,
                'L_ref': 0.424, 'V_ref': 0.576
            },
        ...
        pytest.param(
            {
                ...
                'T': 290.0, 'P': 1.8e6,
                'L_ref': 0.358, 'V_ref': 0.642
            },
            id='ethane-propane-290K-1.8MPa'
        ),
        pytest.param(
            {
                'comp1': 'methane', 'comp2': 'propane',
                'z': np.array([0.55, 0.45]),
                'T': 280.0, 'P': 3e6,
                'L_ref': 0.302, 'V_ref': 0.698
            },
            id='methane-propane-280K-3MPa'
        ),
        pytest.param(
            {
                'comp1': 'methane', 'comp2': 'ethane',
                'z': np.array([0.50, 0.50]),
                'T': 250.0, 'P': 5e6,
                'L_ref': 0.445, 'V_ref': 0.555
            },
            id='methane-ethane-250K-5MPa'
        ),
    ])
    def test_flash_5cases_nist(
        self, flash, compound_critical_properties, test_case
    ):
...
class TestFlashValidationComprehensive:
    ...
    def test_all_flash_reference_points(
        ...
        for test_point in flash_reference_data:
            ...
            if comp1 not in compound_critical_properties or \
               comp2 not in compound_critical_properties:
                continue
...
class TestFlashEdgeCases:
    ...
    def test_near_critical_temperature(
        self, flash, compound_critical_properties
    ):
    ...
    def test_extreme_composition_ethane_rich(
        self, flash, compound_critical_properties
    ):
